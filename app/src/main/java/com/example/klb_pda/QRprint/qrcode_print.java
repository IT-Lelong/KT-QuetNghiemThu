...
public class qrcode_print extends AppCompatActivity  {
    ...
    String res1, res2, res3, res4, res5, res6, name_result;
    DecimalFormat decimalFormat;
    Double g_tyLeChuyenDoi = 0.0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv6 = (TextView) findViewById(R.id.qrcode_print_tv06);
        tv7 = (TextView) findViewById(R.id.qrcode_print_tv07);
        tv8 = (TextView) findViewById(R.id.qrcode_print_tv08);
...
*/
    ...
    public void onDestroy() {
        super.onDestroy();
        unregisterReceiver(receiver);   //掃描機 Máy quét
        mBixolonPrinter.disconnect();
    }
                ...
                case R.id.btnok:
                    try {
                        if (edt1.getText().length() > 0 && edt3.getText().length() > 0 && edt4.getText().length() > 0) {
                            ...
                            ujsonobject.put("ima01", edt1.getText().toString().trim());
                            Thread api = new Thread(new Runnable() {
                                ...
                                public void run() {
                                    if (chkima("http://172.16.40.20/" + g_server + "/QRprint/chkima.php")) {
                                        ...
                                        //數量
                                        String res = getDonVi("http://172.16.40.20/" + g_server + "/PDA_QR230/sosanh_donvi.php?ima01=" + edt1.getText().toString().trim() + "&qty=" + Double.parseDouble(edt3.getText().toString().trim()));
                                        ...
                                        if (res.length() > 0) {
                                            try {
                                                ...
                                                g_tyLeChuyenDoi = 0.0;
                                                g_tyLeChuyenDoi = jsonObject.getDouble("IMA63_FAC"); //轉換率 Tỷ lệ chuyển đổi phát liệu
                                                g_Soluong = jsonObject.getDouble("SL"); //已換算數量 SL đã quy đổi
                                                tv7.setText(String.valueOf(decimalFormat.format(g_Soluong)));
                                                tv8.setText(g_res6);
                ...
                try {
                    ...
                    builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if (qty > Double.parseDouble(edt3.getText().toString().trim())){
                                Toast.makeText(qrcode_print.this, "超出總量 Vượt số lượng tổng", Toast.LENGTH_LONG).show();
                            }else {
    ...
    private void getcode(final String qrcode) {
        ...
        try {
            if (qrcode.startsWith("new")) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (qrcode.substring(4, index2).equals("T")) {
                                    ...
                                    edt2.setText(g_solo);
                                    //數量
                                    String res = getDonVi("http://172.16.40.20/" + g_server + "/PDA_QR230/sosanh_donvi.php?ima01=" + edt1.getText().toString().trim() + "&qty=" + qrcode.substring(index4 + 1).trim());
                                    Double g_Soluong = 0.0;
                                    if (res.length() > 0) {
                                        try {
                                            JSONArray jsonarray = new JSONArray(res);
                                    ...
                                    //數量
                                    String res = getDonVi("http://172.16.40.20/" + g_server + "/PDA_QR230/sosanh_donvi.php?ima01=" + edt1.getText().toString().trim() + "&qty=" + qrcode.substring(index3 + 1).trim());
                                    Double g_Soluong = 0.0;
                                    if (res.length() > 0) {
                                        try {
                                            JSONArray jsonarray = new JSONArray(res);
                ...
                api.start();
            } else if (qrcode.substring(0, 5).equals("CC511") || qrcode.substring(0, 5).equals("CC510") || qrcode.substring(0, 5).equals("CC512") || qrcode.substring(0, 5).equals("CC513") || qrcode.substring(0, 5).equals("CC514") || qrcode.substring(0, 5).equals("CC515") || qrcode.substring(0, 5).equals("CC518") || qrcode.substring(0, 8).equals("OLDSTAMP") || qrcode.substring(0, 8).equals("THUNGHIEM")) {
                Thread api = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                //取得批號
                                String g_solo = "";
                                if (getdatecode("http://172.16.40.20/" + g_server + "/PDA_QR230/getDatecode.php?code=" + qrcode + "&kind=" + 1).equals("NULL")) {
                                    g_solo = "";
                                } else {
                                    g_solo = getdatecode("http://172.16.40.20/" + g_server + "/PDA_QR230/getDatecode.php?code=" + qrcode + "&kind=" + 1);
                                }
                                ;
                                edt2.setText(g_solo);
                                //取得數量
                                if (index2 > 32) {
                                    ...
                                    sl_quydoi = Double.valueOf(l_sl1 + l_sl2 + l_sl3);
                                } else {
                                    sl_quydoi = Double.valueOf(qrcode.substring(index1 + 1, index2).trim());
                                }

                                String res = getDonVi("http://172.16.40.20/" + g_server + "/PDA_QR230/sosanh_donvi.php?ima01=" + edt1.getText().toString().trim() + "&qty=" + sl_quydoi);
                                Double g_Soluong = 0.0;
                                if (res.length() > 0) {
                                    try {
                                        JSONArray jsonarray = new JSONArray(res);
                                        ...
                                        tv7.setText(String.valueOf(decimalFormat.format(g_Soluong)));
                                        tv8.setText(String.valueOf(g_res6));
                                    } catch (JSONException e) {
                                        e.printStackTrace();
                                    }
                                }

                                edt3.setText(String.valueOf(sl_quydoi));
                    ...
                    public void run() {
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                edt3.setText(qrcode.substring(index4 + 1).trim());
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                if (!s.equals("FAIL")) {
                    ...
                    res4 = jsonObject.getString("IMA25"); //庫存單位;
                    res5 = jsonObject.getString("IMA63"); //發料單位;
                    name_result = res1 + "\n" + res2;
        ...
        try {
            ...
            for (int i = 1; i <= num; i++) {
                ...
                if (i == num) {
                    ...
                    db.append(i, xqrb01, xqrb02, String.valueOf(count2), code,g_tyLeChuyenDoi);
                } else {
                    String code = "new_" + xqrb01 + "_" + xqrb02 + "_" + xqrb03;
                    if (xqrb02.equals("NULL")) {
                        xqrb02 = "";
                    }
                    db.append(i, xqrb01, xqrb02, String.valueOf(xqrb03), code,g_tyLeChuyenDoi);
                }
                count2 = count2 - qty;
            }
            runOnUiThread(new Runnable() {
    ...
    private boolean checkdata() {
        try {
            Float count = Float.parseFloat(edt3.getText().toString()); //總數量
            Float qtycount = 0.0f;
            Cursor cursor = db.getAll();
            while (cursor.moveToNext()) {
                float qty = Float.parseFloat(cursor.getString(cursor.getColumnIndexOrThrow("qrb03")));
                qtycount = qtycount + qty;
            }
            cursor.close();
            if (qtycount.equals(count)) {
                return true;
            } else {
                return false;
            }
        ...
        try {
            if (cursor != null && cursor.getCount() >= 0) {
                SimpleCursorAdapter adapter = new SimpleCursorAdapter(this, R.layout.qrcode_print_listview, cursor,
                        new String[]{"qrb00", "qrb01", "qrb02", "qrb03","qrb06"},
                        new int[]{R.id.qrb00, R.id.qrb01, R.id.qrb02, R.id.qrb03, R.id.qrb04}, 0);
    ...
    private void printSample2(final Cursor cursor1) {
        try {
            if (mSampleDialog == null) {
                ...
                cursor1.moveToFirst();
                String xqrb01 = cursor1.getString(cursor1.getColumnIndexOrThrow("qrb01"));
                String xqrb02 = cursor1.getString(cursor1.getColumnIndexOrThrow("qrb02"));
                Double xqrb03 = cursor1.getDouble(cursor1.getColumnIndexOrThrow("qrb03"));
                String xqrb04 = cursor1.getString(cursor1.getColumnIndexOrThrow("qrb04"));
                            ...
                            do {
                                try {
                                    ...
                                    if (xqrb03 > 0) {
                                        ...
                                        mBixolonPrinter.setPrintArea(0, 0, 680, 420);
                                        mBixolonPrinter.setAbsoluteVerticalPrintPosition(340);
